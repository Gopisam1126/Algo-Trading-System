...
| B9 | **`kiteconnect` runs against an `autobahn` it does not declare** | Residual of B7. `kiteconnect` hard-pins `autobahn==19.11.2`, which carries CVE-2020-35678; we install 26.7.1 over it post-resolution, so no environment we build contains the vulnerable version. The pin is declarative rather than a runtime requirement — but the ticker runs against a version its own metadata does not sanction, so a future `kiteconnect` release could change what it needs without the pin telling us. `tests/security/test_dependency_hygiene.py` watches it. Accepted risk, re-check on every `kiteconnect` upgrade. | Watched by tests |
