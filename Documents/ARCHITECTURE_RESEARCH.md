...
- **Three** architectural constraints follow directly from it: **static broker-whitelisted IP**, **India-hosted servers**, and **OAuth + 2FA with daily session logout before pre-open**. These are not paperwork — they shape deployment (LLD §13). *(Corrected 9 Sep 2026: a fourth, “an exchange-assigned Algo-ID on every order”, was listed here and is not a constraint at this operating profile. Registration — and with it the Algo-ID — is conditional on crossing the 10 OPS threshold, SEBI circular 4 Feb 2025 clause I(c). The order-rate cap is what does the work, and it is already listed above.)*
